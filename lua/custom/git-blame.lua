...
local config = {
	enabled = true,
	message_template = "   <sha> •  <author> •  <date> • 󰦨 <summary>",
...
local function time_ago(time)
	...
	if hours < 24 then
		return hours .. (hours == 1 and " hr ago" or " hrs ago")
	end
	...
	if months < 12 then
		return months .. (months == 1 and " mo ago" or " mos ago")
	end
...
local function parse_porcelain(stdout)
	...
	for line in stdout:gmatch("([^\n]+)") do
		...
		if sha then
			...
			if not commit_cache[sha] then
				commit_cache[sha] = { sha = sha }
			end
			local commit = commit_cache[sha]
			for i = 0, c_size - 1 do
				blames[c_final + i] = commit
			end
...
local function format_blame(commit)
	...
	if commit.sha and commit.sha:match("^0+$") then
		return config.message_when_not_committed
	end

	local author = commit.author or "Unknown"
	if current_author and author == current_author then
		author = "You"
	end
...
local function show_blame(bufnr)
	if not config.enabled or nvim_get_current_buf() ~= bufnr or is_insert_mode() then
		return
	end
...
local function fetch_blame(bufnr)
	...
	if vim.tbl_contains(config.ignored_filetypes, vim.bo[bufnr].filetype) then
		return
	end
	...
	local cmd =
		{ "git", "--no-pager", "-C", root, "blame", "-b", "-p", "-w", "--date", "unix", "--contents", "-", filepath }
...
local function queue_fetch(bufnr)
	if not config.enabled or not nvim_buf_is_valid(bufnr) then
		return
	end
	if fetch_timers[bufnr] then
		fetch_timers[bufnr]:stop()
	else
		fetch_timers[bufnr] = uv.new_timer()
	end
	fetch_timers[bufnr]:start(
		config.delay,
		0,
...
function M.setup(opts)
	...
	nvim_create_autocmd("CursorMoved", {
		...
		callback = function(args)
			...
			if cur_row == last_state.row and cur_buf == last_state.buf then
				return
			end
	...
	if config.enabled then
		if not current_author then
			vim.system({ "git", "config", "user.name" }, { text = true }, function(obj)
				if obj.code == 0 and obj.stdout then
					current_author = vim.trim(obj.stdout)
				end
			end)
		end
